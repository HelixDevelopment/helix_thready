...
func mustSemanticEmbedder(cfg EmbedConfig) (Embedder, error) {
    ...
    e := embeddings.New(cfg)                 // digital.vasic.embeddings
    if got := e.Dimension(); got != cfg.ExpectedDim {
...
```go
emb := embeddings.New(embeddings.Config{
    Provider: "openai",                       // interim: OpenAI-compat client…
    BaseURL:  helixLLMURL + "/v1",            // …pointed at HelixLLM (GAP 2.7 interim)
    Model:    "voyage-code-3",
    APIKey:   "local",
})
vecs, err := emb.Embed(ctx, chunks)           // {data:[{embedding,index}]} — sort by index
```
...
**Explanation (for readers/models that cannot see the diagram).** The subsystem has two paths.
...
to pgvector with a cosine index. A **startup guard** branches off the embed step: if the
non-semantic HashEmbedder is configured, the service fails loud rather than indexing garbage —
this is the enforced closure of `[GAP: 2.1]`. On the **read (query) path**, a `/v1/search`
query is embedded with the *same* model, an ANN search returns the top-k nearest chunks from
pgvector, those are reranked with `bge-small-en-v1.5`, the ids are hydrated from the relational
system of record, and `rag` assembles the final context so the answer carries citations back to
the exact posts/materials. The symmetry of the two paths (same model on both sides) is what makes
similarity meaningful; the guard is what stops the whole thing from silently degrading to a hash.
...
- `[OPEN: SEM-1]` The exact `digital.vasic.vectordb` `VectorStore` interface and
  `digital.vasic.embeddings` provider constructor were **not** read field-by-field this pass
  (source fetch failed under rate limit); the `embeddings.New(...)` / `Embed(...)` and pgvector
  DDL shapes are representative and MUST be reconciled with `pkg/vectordb/vectordb.go` +
  `pkg/types/types.go` before implementation. Tracked in the re-verification backlog (gap
  register §13).
