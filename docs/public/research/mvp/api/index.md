<!--
  ...
  Related         : ./openapi.yaml, ./rest-endpoints.md, ./event-bus-contract.md,
                    ./authn-authz.md, ./error-model.md, ./versioning.md, ./sdk-strategy.md,
                    ./contract-tests.md,
...
| File | Purpose |
|------|---------|
| [openapi.yaml](./openapi.yaml) | **The** OpenAPI 3.1 contract for the REST `/v1` surface (43 paths, 53 operations, 37 schemas, webhooks). Schema-first single source of truth. |
...
| [event-bus-contract.md](./event-bus-contract.md) | WebSocket/SSE subscription contract: event envelope, event catalog, subscription patterns, sticky events + invalidation, reconnect/replay, outbound webhooks. |
| [authn-authz.md](./authn-authz.md) | JWT access+refresh (RS256/EdDSA + JWKS), scoped API keys, OAuth2 linking, three-tier RBAC — grounded in `digital.vasic.auth`. |
...
| [versioning.md](./versioning.md) | URL-path `/v1` versioning, additive-vs-breaking policy, `buf` breaking gate, deprecation/Sunset, proto-package parity. |
...
| `#5 / New` Event Bus service (client-facing JetStream subscription) + sticky events | [event-bus-contract.md](./event-bus-contract.md) |
| `#11` SDK generation (OpenAPI 3.1 + Protobuf, helix_proto pattern) | [sdk-strategy.md](./sdk-strategy.md) |
| `#18` TS client libs / scaffolds (no CI, no deep audit) | [sdk-strategy.md](./sdk-strategy.md) |
...
See each file's "Open items" section. Summary:

- `[OPEN: api-1]` The Protobuf event/DTO contract (`proto/helix/thready/v1/*.proto`) is
  specified structurally here but the `.proto` files themselves are produced with the
  code, in the `helix_thready_proto` repo — tracked in [sdk-strategy.md](./sdk-strategy.md).
