...
| 9 | 2026-07-22 | swarm (design · index) | Decision-propagation + delivery pass (operator rulings 2026-07-22, each verified at its owning file): **6 more IDs closed** — `THREADY-DES-03` (heart stays accent green, brand-assets rev 3), `THREADY-DES-08` (desktop v1 = wrapped web only; the desktop mocks are the **v2 contract** — wireframes rev 5 / desktop README rev 2), `THREADY-DES-MKT-01` (3‑page baseline approved, no self-host claim — marketing README rev 2), `THREADY-DES-FIG-01` (**wontfix** — Figma path dropped entirely, kit archived as blueprint proof — figma README rev 3), `THREADY-DES-FIG-02` (moot with `FIG-01`), `THREADY-DES-LIB-02` (**Option B**: Compose‑Multiplatform‑only + thin SwiftUI host — helix-shims-findings rev 2 §5/§6). **4 narrowed / re-scoped**: `THREADY-DES-PLAT-01` (bundle all three faces on all platforms; remaining = per-face OFL verification — typography-substitution rev 2), `THREADY-DES-SCR-MOB-03` (WhatsApp **roadmap-committed**; remaining = the `WA_*` source contract — mobile README rev 3), `THREADY-DES-LIB-01` (React re-bind **executed + merged to main** in `UI-Components-React` per operator ruling — 13 components, 262/262 tests; remaining = the state-completeness re-audit; rebind doc rev 1 predates the merge, lag noted), `THREADY-DES-LIB-03` (inspection **delivered**; remaining = the ArkTS/Aurora client platform decision). New artifact registered: [`platforms/helix-shims-findings.md`](./platforms/helix-shims-findings.md) — first actual `helix_shims` inspection: **no ArkTS/Aurora code, no design/token surface** `[VERIFIED]`. `THREADY-DES-MKT-02` delivery registered (privacy/terms/imprint page designs + `seo.md` — marketing README rev 3). `figma/` + `exports/` catalogue rows re-framed post-wontfix; twins-freshness note added (full-tree regeneration 2026-07-22, final wave 16:31 — 36 md × 2 fresh twins). Totals re-verified by grep: 49 IDs — **40 open / 9 closed**. |
...
archived Figma import kit (blueprint proof), the icon‑export masters + tooling, and the rendered
export package — all
...
| [figma/](./figma/README.md) | **Figma import kit — ARCHIVED (blueprint proof)**: `figma-variables.json` (the full token set as Figma Variables — `Thready / Color` Light+Dark + `Thready / Structure`), `figma-file-plan.md` (8‑page blueprint of the "Thready — Design Library" file). The blueprint is **proven — executed against PenPot** per `[CONSTITUTION §11.4.220]` (evidence `exports/penpot/verification/`); by operator ruling (2026‑07‑22, kit Rev 3) the **Figma path is dropped entirely** — `[CLOSED: THREADY-DES-FIG-01]` **wontfix** (superseding the same-day re-scope) and `[CLOSED: THREADY-DES-FIG-02]` with it. **No Figma file exists and none will be created**; the kit stays on disk as the archived blueprint proof (executing it again would be a new operator decision, not a reopening) |
...
| [platforms/](./platforms/README.md) | **Per-platform customization specs** (deviation contracts, complementing `library/platform-map.md` which maps components and `screens/*/README.md` which carry per-screen chrome): `typography-substitution.md` (per-platform font strategy — bundling **decided 2026‑07‑22**: all three brand faces on all platforms, fallback stacks re-labelled runtime-failure-only; Cyrillic discipline, dynamic-type mapping), `harmonyos.md` + `aurora.md` (dedicated ArkTS / Qt-Silica design contracts; clients are skeletons `[GAP: 8.5]`), `react-token-rebind.md` (remediation contract for `THREADY-DES-LIB-01`/`[GAP: 8.6]` — **executed + merged to main in `UI-Components-React`** per operator ruling 2026‑07‑22: 13 components re-bound, 262/262 tests; the contract doc's rev 1 wording predates the merge, lag noted), and **`helix-shims-findings.md`** (first actual `vasic-digital/helix_shims` inspection, HEAD `a33e902f…`: a tunnel-lifecycle FFI layer — **no ArkTS/Aurora code and no design/token surface of any kind** `[VERIFIED]`; + the §6 SwiftUI decision memo whose Option‑B ruling closed `THREADY-DES-LIB-02`); mints `THREADY-DES-PLAT-01…-08` (`-01` narrowed 2026‑07‑22: remaining = per-face OFL verification) |
...
| [screens/desktop/](./screens/desktop/README.md) | **Desktop (Tauri 2) shell design**: `desktop-shell.html` (per‑OS title bar, menus, tray/notification/file‑drop mocks) + the desktop‑differences catalogue; scope **decided** (operator ruling 2026‑07‑22, `[CLOSED: THREADY-DES-08]`): **v1 ships the wrapped web UI only** — the tray/notification/file‑drop mocks are the **v2 contract** (desktop README rev 2) |
...
| [screens/marketing/](./screens/marketing/README.md) | **Marketing site (Angular 22) designs**: the 3 **approved** pages (`landing` / `features` / `download` — baseline approved 2026‑07‑22, `[CLOSED: THREADY-DES-MKT-01]`, no self-host claim) **+ the rev‑3 legal/SEO delivery**: `privacy.html` / `terms.html` / `imprint.html` **structural page designs** (every legal paragraph an `[OPERATOR — legal counsel text required]` placeholder — no legal fact invented) and [`seo.md`](./screens/marketing/seo.md) (per-page meta/OG/Twitter, doc-backed JSON-LD `SoftwareApplication`, sitemap/robots templates, en/ru/sr‑Cyrl canonical+hreflang); full claims‑traceability register (rows 1–28); `[OPEN: THREADY-DES-MKT-02]` stays open, **actively-scoped** — counsel text, entity details, operator SEO slots, the analytics decision, the store-listing footprint |
...
hand‑off bundles under `exports/`. **PenPot is the sole design platform** `[CONSTITUTION
§11.4.220]`; since the 2026‑07‑22 wontfix ruling (`[CLOSED: THREADY-DES-FIG-01]`) the **Figma
bundle is archived evidence, not a pending hand‑off**. PenPot hand‑off remains **operator‑only**
(no PenPot token in this environment) — see the report's operator‑action list.

**Twin freshness `[CONSTITUTION §11.4.65]`:** the area's `.html`/`.pdf` twins were fully
regenerated 2026‑07‑22 (final wave 16:31); verified on disk at rev 9: every one of the area's
36 `.md` files carries both twins fresher than its source (72 twin files; this index's own
twins regenerated by the rev‑9 pass itself).
...
| **THREADY‑DES‑REACT‑01** | `UI-Components-React` re‑audit + adoption — the token **re‑bind is executed + merged to main** (operator ruling 2026‑07‑22: 13 components re‑bound per the [react-token-rebind.md](./platforms/react-token-rebind.md) contract, 262/262 tests); remaining = the `LIB-01` state‑completeness re‑audit; adoption itself stays "only if a React surface is needed" | `[GAP: 8.6]` | [component-library §10](./component-library.md#10-build-backlog--gaps), [platforms/react-token-rebind.md](./platforms/react-token-rebind.md) |
...
Re‑verified against the tree on 2026‑07‑22 (rev 9) by grepping `OPEN: THREADY-…` / `CLOSED:
THREADY-…` over `design/`: every ID below exists at its owning file, and no ID exists in the
tree that is absent below (49 unique IDs both ways). Closure state is authoritative **per
owning file**; non‑owning files may still carry pre‑ruling `[OPEN: …]` markers for
newly‑closed IDs (revision‑history rows and `figma-file-plan.md`'s verbatim blueprint markers
are historical by design; genuinely stale: `design-system.md` / `assets/icon-export-matrix.md`
/ `screens/web/README.md` for `DES-03`, `DESIGN_PACKAGE_REPORT.md` for `DES-08`,
`library/platform-map.md` §6 + `tokens-bridge/README.md` for the `LIB` family — owner passes
pending, noted per row). **State summary: 40 open (narrowed / re‑scoped: `THREADY-MOT-03`,
`THREADY-DES-LIB-01`, `THREADY-DES-LIB-03`, `THREADY-DES-LIB-04`, `THREADY-DES-PLAT-01`,
`THREADY-DES-SCR-MOB-03`; `THREADY-DES-02` advanced, `THREADY-DES-MKT-02` actively‑scoped),
9 closed (`THREADY-DES-01`, `THREADY-DES-03`, `THREADY-DES-08`, `THREADY-DES-OD-03`,
`THREADY-DES-FIG-01` wontfix, `THREADY-DES-FIG-02`, `THREADY-DES-FIG-03`,
`THREADY-DES-MKT-01`, `THREADY-DES-LIB-02`).**
...
| **THREADY-DES-03** | `CLOSED` *(2026‑07‑22)* | Heart‑glyph color **decided** (operator ruling): stays `--ds-heart: var(--accent)` — accent green, AA‑legible in both modes, white‑label‑safe (a rebranded Account's validated accent re‑tints the heart; a hard‑coded love‑red would sit outside the override contract and risk reading as `--danger`); the classic love‑red alternative is **dropped**. Recorded in brand‑assets rev 3 (§8/§11); `design-system.md` §3.2/§10, `assets/icon-export-matrix.md` and `screens/web/README.md` still carry the pre‑ruling `[OPEN]` marker — staleness noted, owner pass pending | [brand-assets §8](./brand-assets.md#8-the-footer-slogan--heart-glyph), [design-system §3.2](./design-system.md#32-the-thready-brand-theme) |
...
| **THREADY-DES-08** | `CLOSED` *(2026‑07‑22)* | **Decided** (operator ruling): desktop (Tauri) **v1 ships the wrapped web UI only** — tray, native notifications and file‑drop are designed‑but‑deferred; the [screens/desktop/](./screens/desktop/README.md) mocks are the **v2 contract**, not v1 scope (wireframes rev 5 §7, desktop README rev 2; `DESIGN_PACKAGE_REPORT.md` still carries the pre‑ruling marker — staleness noted) | [wireframes §7](./wireframes.md#7-gaps--open-items), [screens/desktop/README](./screens/desktop/README.md) |
...
| ID | State | Summary | Owning file(s) |
|----|-------|---------|----------------|
| **THREADY-DES-LIB-01** | `OPEN` *(narrowed 2026‑07‑22)* | React re‑audit (`[GAP: 8.6]`) — the **styling half is done**: the [react-token-rebind.md](./platforms/react-token-rebind.md) contract was **executed in `UI-Components-React` and merged to main** per operator ruling 2026‑07‑22 (13 components re‑bound to the token vars, 262/262 tests; executed status referenced by [helix-shims-findings §5/§6](./platforms/helix-shims-findings.md)). Remaining: the **state‑completeness re‑audit** (rebind contract §3 criterion 5 — confirm each of the 13 files implements the library's hover/focus/disabled/loading/error states) before any React surface consumes the package. The rebind doc (rev 1) and [platform-map §6](./library/platform-map.md#6-open-items) predate the merge — owning‑file lag noted | [platforms/react-token-rebind.md §3](./platforms/react-token-rebind.md#3-acceptance-criteria-closes-the-styling-half-of-the-gap), [platform-map §6](./library/platform-map.md#6-open-items) |
| **THREADY-DES-LIB-02** | `CLOSED` *(2026‑07‑22)* | iOS host **decided** (operator ruling): **Option B — Compose‑Multiplatform‑only with a thin SwiftUI host shell**, per the [helix-shims-findings §6](./platforms/helix-shims-findings.md#6-swiftui-decision-memo-thready-des-lib-02) decision memo (in‑house thin‑Swift‑host precedent: `helix_shims` `apple/ios-spike`; iOS idiom deviations accepted as documented). **No dedicated SwiftUI package will be built**; the generated `ThreadyTokens.swift` contract stays on the shelf as labeled. Remaining engineering dependency (not this item): the `UI-Components-KMP` Yole→Thready retheme consuming `ThreadyColors.kt`. [platform-map §6](./library/platform-map.md#6-open-items) + `tokens-bridge/README.md` markers predate the ruling — lag noted | [helix-shims-findings §5/§6](./platforms/helix-shims-findings.md#5-status-of-the-workable-items-thready-des-lib-02---03), [platform-map §6](./library/platform-map.md#6-open-items) |
| **THREADY-DES-LIB-03** | `OPEN` *(narrowed 2026‑07‑22)* | **Inspection half delivered**: [helix-shims-findings.md](./platforms/helix-shims-findings.md) records the first actual `helix_shims` inspection (HEAD `a33e902f…`, 2026‑07‑22) — a tunnel‑lifecycle FFI layer (flutter_rust_bridge / JNI / C‑ABI) with **no ArkTS/Aurora code and no design/token surface of any kind** `[VERIFIED — grep sweep + full tree walk]`; tokens will never flow *through* it, so the ArkTS/Qt token path is independent of the shim contracts. Remaining, narrowed: the **actual ArkTS/Aurora client platform decision** (whether/when native HarmonyOS + Aurora clients exist to consume the contract‑only token bindings); the AR/QT shim halves are `[BUILD-NEW]`, owned by the client program. [platform-map §6](./library/platform-map.md#6-open-items)'s "not inspected" wording predates the findings — lag noted | [helix-shims-findings §5](./platforms/helix-shims-findings.md#5-status-of-the-workable-items-thready-des-lib-02---03), [platform-map §6](./library/platform-map.md#6-open-items) |
...
| **THREADY-DES-SCR-MOB-03** | `OPEN` *(re‑scoped 2026‑07‑22)* | **WhatsApp as a third Add‑Channel source — decided: roadmap‑committed** (operator ruling; mobile README rev 3). No longer a question of *whether*: the WhatsApp adapter is a committed roadmap item; the tile in `add-channel.html` **stays disabled until the adapter exists** (WhatsApp remains absent from the verified source set — `TG_*`/`MAX_*` only). Remaining under this ID: design the **`WA_*` source contract** (env‑contract shape mirroring `TG_*`/`MAX_*`, wizard states, auth flow) so the tile can light up when the adapter lands | [screens/mobile/README §4](./screens/mobile/README.md#4-open-items) |
...
| ID | State | Summary | Owning file(s) |
|----|-------|---------|----------------|
| **THREADY-DES-MKT-01** | `CLOSED` *(2026‑07‑22)* | **Final marketing IA / copy sign‑off — approved** (operator ruling): the 3‑page baseline (`landing`/`features`/`download`), its IA and its copy are the **approved MVP marketing design**; the self‑host question resolves with it — the approved baseline makes **no self‑host claim**. Ordinary copy iteration stays possible; the sign‑off is no longer pending (marketing README rev 2, §7) | [screens/marketing/README §7](./screens/marketing/README.md#7-open-items) |
| **THREADY-DES-MKT-02** | `OPEN` *(actively‑scoped 2026‑07‑22)* | **SEO / analytics / legal pages scope — design delivered, text is counsel's** (marketing README rev 3): `privacy.html` / `terms.html` / `imprint.html` structural page designs + [`seo.md`](./screens/marketing/seo.md) (per‑page meta/OG/Twitter, doc‑backed JSON‑LD, sitemap/robots, hreflang) landed — no legal fact invented. Remaining: **counsel text** (every legal paragraph is an `[OPERATOR — legal counsel text required]` block), **legal entity details**, final locale URLs + the operator SEO slots (origin, og:locale, social handles, JSON‑LD org URL), the **analytics decision** (adoption or deliberate absence), and the **store‑listing legal footprint** | [screens/marketing/README §7](./screens/marketing/README.md#7-open-items), [seo.md](./screens/marketing/seo.md) |
...
### Figma kit (`THREADY-DES-FIG-*`) — [figma/](./figma/README.md) — archived; family fully closed

| ID | State | Summary | Owning file(s) |
|----|-------|---------|----------------|
| **THREADY-DES-FIG-01** | `CLOSED` *(wontfix 2026‑07‑22)* | **Figma path dropped entirely** (operator ruling, superseding the same‑day re‑scope — the re‑scope history is preserved in the kit's revision table): **PenPot is the sole design platform** `[CONSTITUTION §11.4.220]`; **no Figma file exists and none will be created**. The kit's blueprint was already **proven via the PenPot materialization** (evidence `exports/penpot/verification/`), so `figma/` stays on disk as the **archived blueprint proof** (kit Rev 3). Executing the archived path would be a new operator decision, not a reopening | [figma/README](./figma/README.md#open-items) |
| **THREADY-DES-FIG-02** | `CLOSED` *(2026‑07‑22)* | Variables import mechanism (Enterprise REST bulk endpoint vs. Plugin‑API replay vs. import plugin) — **moot**: closes with `FIG-01` wontfix, since it was conditioned entirely on `FIG-01`'s optional import being exercised | [figma/README](./figma/README.md#open-items) |
...
| ID | State | Summary | Owning file(s) |
|----|-------|---------|----------------|
| **THREADY-DES-PLAT-01** | `OPEN` *(narrowed 2026‑07‑22)* | **Font bundling — decided** (operator ruling; typography‑substitution rev 2): **bundle all three brand faces on all platforms** (Android/iOS/HarmonyOS/Aurora); the §3 fallback stacks are re‑labelled **runtime‑failure fallbacks only**, no longer the shipping default. Remaining scope: per‑face **license/redistribution (OFL) verification** before any bundle ships (the variable‑vs‑static‑instances choice is an implementation detail of the decided bundling, not part of this item) | [platforms/typography-substitution §7](./platforms/typography-substitution.md#7-open-items) |
