...
/* 4.1 W3C DTCG design tokens (also the PenPot 2.x design-tokens import file) */
function emitDtcg(model) {
  const doc = {
    $description:
      'GENERATED — DO NOT EDIT. Helix Thready design tokens in W3C DTCG format ' +
      '(PenPot 2.x design-tokens import file). Generated by tokens-bridge/generate.mjs ' +
  ...
  doc['thready-structure'] = structure;
  return {
    relPath: 'web/tokens.json',
    content: JSON.stringify(doc, null, 2) + '\n',
    colors: modelColorSet(model),
    extractColors: (s) => new Set([...s.matchAll(/"\$value": "#([0-9a-fA-F]{6})"/g)].map((m) => up(m[1]))),
    symbolRe: /"\$type":/g,
    symbols,
  };
}
...
function runChecks(targets, model) {
  ...
  // (d) DTCG parses as JSON
  const json = targets.find((t) => t.relPath === 'web/tokens.json');
  try {
    JSON.parse(json.content);
    ok('web/tokens.json: JSON.parse', true);
  } catch (e) {
    ok('web/tokens.json: JSON.parse', false, String(e));
  }
