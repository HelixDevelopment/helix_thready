...
// raw request body. It matches the event-bus contract §9 ("X-Thready-Signature").
const SignatureHeader = "X-Thready-Signature"

// signaturePrefix is prepended to the hex digest, mirroring the common
// "sha256=<hex>" webhook convention.
...
// Verify recomputes the HMAC over body and constant-time compares it to the
// value from the SignatureHeader (with or without the "sha256=" prefix).
...
func (w *WebhookSink) Notify(ctx context.Context, env Envelope) error {
	...
	if err != nil {
		return fmt.Errorf("callbacktask: marshal envelope: %w", err)
	}
...
// deliver POSTs body with the HMAC signature header, retrying on non-2xx or
// transport error. The signature is computed over the exact bytes sent, so the
// receiver can recompute and match it.
func (w *WebhookSink) deliver(ctx context.Context, body []byte) error {
	...
	for attempt := 0; attempt <= w.MaxRetries; attempt++ {
		...
		if err != nil {
			return fmt.Errorf("callbacktask: build request: %w", err)
		}
		...
		lastErr = fmt.Errorf("callbacktask: webhook %s returned HTTP %d", w.URL, resp.StatusCode)
	}

	return fmt.Errorf("callbacktask: delivery failed after %d attempt(s): %w", w.MaxRetries+1, lastErr)
}
